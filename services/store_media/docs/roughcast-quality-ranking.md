...
`bizcircle` **第 1 期即填**：原始行里确实有 `bizCircleName`
（`crm_connector/run/probe_rental_row.out.json` 已确认），connector 的
**改动 E 已于 2026-08-20 落地**，赶在第 1 轮真跑之前，所以该列从第一轮起就有值。

选在第 1 轮之前落地，理由**不是**「早一天少一天 NULL」——队列 A 每天重扫全量，
且 `_refresh_current` 按 `ON CONFLICT DO UPDATE SET` 更新全部业务列，E 无论何时
落地，之后的第一轮都会把全部活跃房源回填，不存在越欠越多的 NULL。真正的理由是
出口条件要求「连续 3 天产出 COMPLETE run」，而 E 需要重启 connector：放在 3 天
窗口中间，观测的就不是同一套配置了。

它是 2.4.2 门槛 2（同商圈）的输入。注意它**不在 `HASH_FIELDS`** 里，所以加它
不会把每一行都判成变更点——否则 E 落地后的第一轮会让快照表凭空翻一倍。
...
7. ~~**清水房总数是否稳定在 3000 上下**~~ **量级已确认，稳定性待观察**。
   **注意：本条下方 2026-08-21 的更正表明 `total` 并非清水房数，先读那段再用这里的数。**
   ...
   **已有第三个数据点**：`3289` → `3290`（同日稍后）→ **`3291`**（第 2 期探针，同日）。
   ...
   ⚠️ **`total` 不是清水房数——2026-08-21 第 1 轮全量实测推翻了本条的原有前提。**
   run 1 走满 66 页、`has_more` 正好在第 66 页转 false（`pages_done ==
   pages_expected`，无分歧告警），但全程只收到 **998** 行，平均 15 行/页而非 50。
   `non_roughcast_count = 0` 证明过滤是上游做的、不是我们本地做的。
   结论：**上游把 `fitment=002` 应用在分页之后**——`total`(3290) 是过滤前的在租
   总数，每页先切 50 条再过滤，存活约 30%（998/3290 = 30.3%，恰合 15/50）。

   因此两个数必须分开记、分开用，**不可互相替代**：

   | 数 | 本轮值 | 决定什么 | 谁消费 |
   | --- | --- | --- | --- |
   | `upstream_total` | 3290 | `pages_expected`、节奏、预算 | 第三章、4.2 的 COMPLETE 判据 |
   | 毛坯数（发布数） | 998 | 被排序集 P 的大小 | 第 3 期覆盖率分母、2.2 阶梯定稿 |

   66 页必须走满（清水房散落在全部页里），所以第三章的预算模型不受影响，
   67 次请求对 260 硬顶依然宽裕。但**拿 `total` 当 P 的大小会高估 3.3 倍**，
   第 3 期定阶梯前务必用毛坯数。`--status` 两列均已落地，无需改实现。

   ⚠️ **3 天窗口的抽样偏差**：run 1 落在 **2026-08-21 周五**，按原计划第 2、3 天为
   **周六、周日**——3 天里 2 天是周末。若挂牌/下架存在工作日–周末节律（中介周末
   带看、周一集中录入），则本窗口量到的"稳定"可能只是周末稳定，而最可能出现批量
   变动的**周一恰好落在窗口外**——那正是本条最想防的跳变。
   建议补到周一（多 67 次请求，日额度独立）以覆盖一次周末→工作日跨越；
   若仍按 3 天收口，**此偏差必须记录，不得静默**（与第 4 条截断同一原则）。
